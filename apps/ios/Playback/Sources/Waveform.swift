        ...
        GeometryReader { g in
            ...
            ZStack(alignment: .leading) {
                ...
                ForEach(noteMarks) { m in
                    Circle()
                        .fill(m.resolved ? PB.green : PB.redline)
                        .frame(width: 6, height: 6)
                        .overlay(Circle().strokeBorder(PB.black.opacity(0.4), lineWidth: 0.5))
                ...
                // note marker
                ZStack {
                    Rectangle().fill(PB.cobalt).frame(width: 2, height: h)
                    Circle().fill(PB.cobalt).frame(width: 9, height: 9).offset(y: -h / 2)
                    Circle().fill(PB.cobalt).frame(width: 9, height: 9).offset(y: h / 2)
                }
                .offset(x: w * marker - 1)
            }
