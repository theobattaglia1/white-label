...
/// Playback · iMessage Extension entry point.
///
...
///     which creates an MSMessage whose URL encodes the Playback share token.
...
final class MessagesViewController: MSMessagesAppViewController {
    ...
    private func sendReply(body: String, timestampMS: Int) {
        ...
        Task {
            ...
            do {
                _ = try await PlaybackReceiptAPI.shared.postNote(
                    token: token,
                    body: body,
    ...
    private func sendApproval() {
        ...
        Task {
            do {
                _ = try await PlaybackReceiptAPI.shared.approve(versionID: versionID)
...
final class ReceiptModel: ObservableObject {
    ...
    /// Decode the URL the sender's app encoded: playback://r/<token> or
    /// legacy wl://r/<token>. The query may carry additional hints (title etc.)
    func handleIncomingURL(_ url: URL) {
        ...
        Task {
            do {
                let payload = try await PlaybackReceiptAPI.shared.shared(token: token)
...
struct ReceiptHost: View {
    ...
    var body: some View {
        ZStack {
            ReceiptColors.sleeveCream.ignoresSafeArea()
    ...
    private func cover(size: CGFloat) -> some View {
        ...
        .overlay(alignment: .bottomLeading) {
            HStack(spacing: 2) {
                Text("P").font(.system(size: size * 0.18, weight: .black, design: .default).width(.condensed)).foregroundStyle(.white)
