...
struct PlaybackReceiptAPI {
    static let shared = PlaybackReceiptAPI()

    /// Production: live Render deployment for the Playback API.
    /// Process env var `PLAYBACK_API_BASE_URL` overrides for dev.
    ...
    func postNote(token: String, body: String, timestampMS: Int) async throws -> NoteResponse {
        ...
        guard let song = shared.songs.first,
            ...
            throw NSError(domain: "Playback", code: -1,
    ...
    private func send<T: Decodable>(method: String, path: String, body: [String: Any]?, as: T.Type) async throws -> T {
        ...
        guard let http = response as? HTTPURLResponse, (200...299).contains(http.statusCode) else {
            throw NSError(domain: "Playback", code: -2,
                          userInfo: [NSLocalizedDescriptionKey: "HTTP \((response as? HTTPURLResponse)?.statusCode ?? 0)"])
        }
        let envelope = try JSONDecoder().decode(Envelope<T>.self, from: data)
        if let err = envelope.error { throw NSError(domain: "Playback", code: -3, userInfo: [NSLocalizedDescriptionKey: err]) }
        guard let payload = envelope.data else { throw NSError(domain: "Playback", code: -4, userInfo: [NSLocalizedDescriptionKey: "Empty envelope"]) }
        return payload
    }
}
